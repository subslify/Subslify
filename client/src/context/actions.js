const DISPLAY_ALERT = 'DISPLAY_ALERT';
const REMOVE_ALERT = 'REMOVE_ALERT';

const REGISTER_USER_BEGIN = 'REGISTER_USER_BEGIN';
const REGISTER_USER_SUCCESS = 'REGISTER_USER_SUCCESS';
const REGISTER_USER_ERROR = 'REGISTER_USER_ERROR';

const LOGIN_USER_BEGIN = 'LOGIN_USER_BEGIN';
const LOGIN_USER_SUCCESS = 'LOGIN_USER_SUCCESS';
const LOGIN_USER_ERROR = 'LOGIN_USER_ERROR';

const LOGOUT_USER = 'LOGOUT_USER';

const UPDATE_USER_BEGIN = 'UPDATE_USER_BEGIN';
const UPDATE_USER_SUCCESS = 'UPDATE_USER_SUCCESS';
const UPDATE_USER_ERROR = 'UPDATE_USER_ERROR';

const GET_CURRENT_USER_BEGIN = 'GET_CURRENT_USER_BEGIN';
const GET_CURRENT_USER_SUCCESS = 'GET_CURRENT_USER_SUCCESS';

const TOGGLE_SIDEBAR = 'TOGGLE_SIDEBAR';

//Subscriptions
const GET_SUBSCRIPTIONS_SUCCESS = 'GET_SUBSCRIPTIONS_SUCCESS';
const GET_SUBSCRIPTIONS_ERROR = 'GET_SUBSCRIPTIONS_ERROR';
const GET_SUBSCRIPTIONS_BEGIN = 'GET_SUBSCRIPTIONS_BEGIN';

const CLEAR_VALUES = 'CLEAR_VALUES';
const HANDLE_CHANGE = 'HANDLE_CHANGE';
const CREATE_SUBSCRIPTION_BEGIN = 'CREATE_SUBSCRIPTION_BEGIN';
const CREATE_SUBSCRIPTION_SUCCESS = 'CREATE_SUBSCRIPTION_SUCCESS';
const CREATE_SUBSCRIPTION_ERROR = 'CREATE_SUBSCRIPTION_ERROR';

const DELETE_SUBSCRIPTION_BEGIN = 'DELETE_SUBSCRIPTION_BEGIN';
const SET_EDIT_SUBSCRIPTION = 'SET_EDIT_SUBSCRIPTION';

const EDIT_SUBSCRIPTION_BEGIN = 'EDIT_SUBSCRIPTION_BEGIN';
const EDIT_SUBSCRIPTION_SUCCESS = 'EDIT_SUBSCRIPTION_SUCCESS';
const EDIT_SUBSCRIPTION_ERROR = 'EDIT_SUBSCRIPTION_ERROR';

export {
  DISPLAY_ALERT,
  REMOVE_ALERT,
  REGISTER_USER_BEGIN,
  REGISTER_USER_SUCCESS,
  REGISTER_USER_ERROR,
  LOGIN_USER_BEGIN,
  LOGIN_USER_SUCCESS,
  LOGIN_USER_ERROR,
  LOGOUT_USER,
  UPDATE_USER_BEGIN,
  UPDATE_USER_SUCCESS,
  UPDATE_USER_ERROR,
  GET_CURRENT_USER_BEGIN,
  GET_CURRENT_USER_SUCCESS,
  TOGGLE_SIDEBAR,
  GET_SUBSCRIPTIONS_SUCCESS,
  GET_SUBSCRIPTIONS_ERROR,
  GET_SUBSCRIPTIONS_BEGIN,
  CLEAR_VALUES,
  HANDLE_CHANGE,
  CREATE_SUBSCRIPTION_BEGIN,
  CREATE_SUBSCRIPTION_SUCCESS,
  CREATE_SUBSCRIPTION_ERROR,
  DELETE_SUBSCRIPTION_BEGIN,
  EDIT_SUBSCRIPTION_BEGIN,
  EDIT_SUBSCRIPTION_SUCCESS,
  EDIT_SUBSCRIPTION_ERROR,
  SET_EDIT_SUBSCRIPTION,
};
